...
- The **result block** — the delimited JSON a Role is instructed to end with —
  belongs to AgentBastion. It travels in the prompt, so every adapter gets it for
...
class CliProvider(Provider):
    ...
    def preflight(self) -> None:
        try:
            require(
                self.runner,
                self.binary,
                f"AgentBastion drives the {self.name} CLI (ADR-0001); install it or "
    ...
    def _deliver_skills(self, role: Role, prompt: str) -> tuple[str, tuple[str, ...]]:
        ...
        if self.capability_tier is CapabilityTier.NATIVE:
            ...
            commands = ", ".join(f"/agentbastion:{name}" for name in role.skills)
            instruction = (
                f"Use the declared native AgentBastion skills before doing this work: {commands}."
            )
    ...
    if payload is None:
        return AgentResult(
            ...
            summary=(
                ...
                "so AgentBastion cannot tell what it did"
            ),
            detail=output.text,
            raw=output.text,
            usage=output.usage,
