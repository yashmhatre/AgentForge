...
Model identifiers appear here and nowhere else in AgentBastion (ADR-0004). The
aliases are used rather than pinned versions so that a vendor release does not
require an AgentBastion release; a team that wants a specific version overrides
the mapping in configuration once `agentbastion init` exists (M5).
"""
...
def _usage(record: dict) -> Usage | None:
    ...
    AgentBastion was already parsing for the result text. `None` when the envelope
