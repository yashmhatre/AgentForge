...
INSTRUCTIONS = """\
You are the Tester in AgentBastion. Work from the frozen Plan, not from the human's \
...
You are on the same branch the Implementer used. Commit nothing; AgentBastion \
        ...
        if not getattr(self.provider, "allow_commands", False):
            return AgentResult(
                ...
                detail=(
                    ...
                    "`agentbastion implement` with `--allow-commands`."
                ),
            )
