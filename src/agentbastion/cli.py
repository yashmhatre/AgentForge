"""Command-line entry point for AgentBastion."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init = subcommands.add_parser(
        "init",
        help="not built yet (M5): configure AgentBastion for the repository in this directory",
    )
...
def _run_plan(args: argparse.Namespace, runner=None) -> int:
    from .core.runtime import Bastion, RunFailed
    from .providers import DEFAULT_PROVIDER

    bastion = Bastion(cwd=args.directory, provider=args.provider or DEFAULT_PROVIDER, runner=runner)

    try:
        outcome = bastion.plan(
            args.task,
            tier=_tier(args.tier) if args.tier else None,
            interviewer=build_interviewer(),
        )
    except RunFailed as exc:
        print(f"agentbastion: {exc}", file=sys.stderr)
        return 2

    result = outcome.result
    if not outcome.filed:
        heading = "needs a decision from you" if result.escalated else "could not plan this task"
        print(f"agentbastion: the Orchestrator {heading}.", file=sys.stderr)
    ...
    print(f"\nRun it with:  agentbastion implement {issue.number}")
    return 0
...
def _run_implement(args: argparse.Namespace, runner=None) -> int:
    ...
    if state.status is RunStatus.AWAITING_SIGNOFF:
        ...
        print("AgentBastion stops at Sign-off. A human merges.")
        return 0
...
def main(argv: list[str] | None = None, runner=None) -> int:
    ...
    raise SystemExit(f"agentbastion {args.command} is not implemented yet.")
