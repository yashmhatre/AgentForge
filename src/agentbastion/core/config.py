...
def load_config(root: Path | str) -> Config:
    """Read ``.agentbastion/config.yaml``, or return documented defaults."""
    path = Path(root) / ".agentbastion" / "config.yaml"
    if not path.is_file():
        return Config()
...
def _test_suite(data: dict) -> tuple[str, ...]:
    ...
    by the project, run by AgentBastion itself rather than by a Role.
    """
