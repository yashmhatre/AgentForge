...
Markdown. `agentbastion implement` recovers the Plan a week later on a different
...
PLAN_OPEN = "<!-- agentbastion:plan -->"
PLAN_CLOSE = "<!-- /agentbastion:plan -->"

RESULT_OPEN = "<!-- agentbastion:result -->"
RESULT_CLOSE = "<!-- /agentbastion:result -->"
...
GATE_OPEN = "<!-- agentbastion:gate -->"
GATE_CLOSE = "<!-- /agentbastion:gate -->"


class PlanFormatError(ValueError):
    """An Issue body does not carry a plan AgentBastion can execute."""
...
def render_issue_body(task: Task, document: PlanDocument) -> str:
    ...
    parts += [
        ...
        "",
        (
            "*Filed by AgentBastion. The block above is the frozen execution contract "
...
def extract_plan_payload(text: str) -> dict:
    ...
    if payload is None:
        raise PlanFormatError(
            "no AgentBastion plan block found; "
            "it was not written by `agentbastion plan`, or the block was deleted"
        )
...
def parse_issue_body(body: str, resolve=None) -> PlanDocument:
    ...
    if version > PLAN_FORMAT_VERSION:
        raise PlanFormatError(
            f"issue carries plan format v{version}; this AgentBastion understands "
            f"v{PLAN_FORMAT_VERSION}. Upgrade AgentBastion."
        )
...
def render_gate_block(payload: dict) -> str:
    """The block a Gate ends its Run Log entry with. Written by AgentBastion only.
