...
def branch_for_issue(issue: int) -> str:
    ...
    """
    return f"agentbastion/issue-{issue}"


@dataclass(frozen=True)
class Repository:
    """A git working tree AgentBastion is allowed to act on."""
...
def open_repository(runner: CommandRunner, cwd: Path | str) -> Repository:
    ...
    try:
        require(runner, "git", "AgentBastion drives git rather than reimplementing it.")
    ...
    if not remote:
        raise PreconditionFailed(
            f"{repo.root} has no `origin` remote. AgentBastion hands off through GitHub "
