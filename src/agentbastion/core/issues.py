...
Every GitHub call in AgentBastion is in this file, made by invoking `gh` through
the Command Runner. AgentBastion implements no GitHub authentication and speaks to
...
@dataclass(frozen=True)
class Issue:
    """A GitHub issue as AgentBastion reads it."""
...
class GitHub:
    ...
    def ensure_label(self, label: str) -> None:
        ...
        self._gh("label", "create", label, "--description", "AgentBastion run status", check=False)
    ...
    if result.outcome is Outcome.ESCALATED:
        lines += [
            ...
            (
                ...
                "and re-run `agentbastion implement`."
            ),
        ]
    ...
    if entry.blocked and entry.invalidates:
        lines += [
            ...
            (
                ...
                "so that Step is marked for re-run: the next `agentbastion implement` runs it "
...
def _what_next(state: RunState) -> str:
    ...
    rerun = f"`agentbastion implement {state.issue}`"
    if state.status is RunStatus.AWAITING_SIGNOFF:
        return "AgentBastion stops here. Sign-off is a human Gate; no Workflow merges."
    ...
    return (
        "AgentBastion could not finish this Run. The Run Log entry above carries the detail; "
...
def run_state(issue: Issue, resolve=None) -> RunState:
    ...
    status. Nothing local is consulted, which is why `agentbastion implement 12`
