"""The shared vocabulary of AgentBastion, as data.
...
class RunStatus(StrEnum):
    ...
    human decides. ``FAILED`` is a Run AgentBastion could not finish at all.

    ``HALTED`` was ``ESCALATED`` until the glossary settled that an Escalation
    is the verdict a Role reports and Halted the state it produces. Nothing
    reads that older spelling any more; see ``LEGACY_LABELS`` for why.
    """
    ...
    @property
    def label(self) -> str:
        return f"agentbastion:{self.value}"


#: Labels an earlier AgentBastion applied, and what they mean now. Read, never
#: written, so that a Run can still read its own state back off an Issue that
#: was open when a label was renamed — the one thing ADR-0002 does not survive.
#:
#: Empty, deliberately. Two renames could have put entries here and neither
#: does. `agentforge:escalated` was carried for the Escalated-to-Halted rename;
#: by the time ADR-0012 landed no Issue in this repository or the smoke
#: repository still wore it, so it was retired rather than carried further. The
#: `agentforge:*` status labels this project wrote before ADR-0012 are not read
#: back either: exactly one Issue wore one, in the smoke repository, and it was
#: relabelled by hand. Nothing outside these two repositories has ever seen
#: either spelling, and reading a form forever that nobody else ever wrote is
#: the bill ADR-0012 renamed early in order not to pay.
#:
#: The mechanism stays for the rename that does land with Issues open.
LEGACY_LABELS: dict[str, RunStatus] = {}

#: Every status label AgentBastion may find on an Issue, so a caller can reconcile
...
class RunState:
    ...
    def done_roles(self) -> tuple[str, ...]:
        ...
        `agentbastion implement` again, and the Role that escalated is the one
