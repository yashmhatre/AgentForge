...
class PlanOutcome:
    """What `agentbastion plan` produced."""
...
class Bastion:
    ...
    def implement(
        ...
        if repo.is_dirty():
            raise RunFailed(
                f"{repo.root} has uncommitted changes. AgentBastion commits whatever an Agent "
        ...
        # AgentBastion did not write. What none of those may do is claim success
...
def _pr_body(number: int, state: RunState, results, changed) -> str:
    ...
    lines += [
        ...
        "---",
        "",
        "Opened as a draft by AgentBastion. A human merges; no Workflow does.",
    ]
    return "\n".join(lines) + "\n"
...
__all__ = ["Bastion", "PlanOutcome", "RunFailed"]
