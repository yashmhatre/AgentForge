...
A Gate that blocks on a Role's output has to mark that Role's Step for re-run, or the Run deadlocks: Security finds something, the Gate blocks, a human fixes the code, and the next `agentbastion implement` re-runs no completed Step — so the Gate re-reads the same stale finding and blocks again, forever. ADR-0002 leaves exactly one place to record that a Step was invalidated, because Run State is derived from the Issue and nothing else. So a Gate posts its verdict to the Run Log as a comment carrying an `agentbastion:gate` block, and `RunState.done_roles` un-retires a Step when a blocked verdict names the Role it was drawn from.
...
Only Gate verdicts and Agent Results are replayed, so the two markers are now a compatibility surface as much as the plan block is: a Run Log written today is read by an `agentbastion implement` running next month.
