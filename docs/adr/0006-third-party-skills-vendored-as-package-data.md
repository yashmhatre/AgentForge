...
AgentBastion depends on five skills it does not own, and they are agent instructions and loose scripts rather than published packages — there is nothing to declare as a dependency. They are vendored into `src/agentbastion/skills/` as package data, invoked as subprocesses through the Command Runner and never imported, with upstream repository, commit SHA, licence, and deliberate exclusions recorded in `skills/MANIFEST.yaml` so that a reader finding someone else's MIT code in the source tree learns why instead of removing it.
