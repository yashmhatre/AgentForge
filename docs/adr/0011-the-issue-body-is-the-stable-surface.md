...
At 0.1 AgentForge promises that what it writes into a GitHub issue keeps parsing, and promises nothing about what `import agentforge_framework` gives you.
...
Everything importable under `agentforge_framework.*` is private, and will change without notice. AgentForge is driven through a command line and a tracker; nothing about it is designed to be called. The internal seams are also the parts most likely to move: the Provider port grows a third adapter, the Gate registry and the extractor registry both open up to Plugins, and `agentforge init` rewrites how configuration is read. Freezing those at 0.1 would freeze the work that has not been done yet.
...
A change to any of the four blocks or to the label scheme is a compatibility decision, and the version rule above is how it gets made. A change under `agentforge_framework.*` is not, however public the name looks.
